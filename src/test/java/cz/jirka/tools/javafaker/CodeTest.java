...
public class CodeTest extends AbstractFakerTest {
    ...
    public void testOverrides() {
        ...
        assertThat("Uses overridden expressions from test.yml", 
                isbn10Sep, 
                MatchesRegularExpression.matchesRegularExpression("9971-\\d-\\d{4}-(\\d|X)"));
        
        assertThat("Uses overridden expressions from test.yml",
                isbn13Sep, 
                MatchesRegularExpression.matchesRegularExpression("(333|444)-9971-\\d-\\d{4}-\\d"));
    }

    @Test
    public void asin() {
        assertThat(faker.code().asin(), MatchesRegularExpression.matchesRegularExpression("B000([A-Z]|\\d){6}"));
    }
    ...
    public void imei() {
        ...
        assertThat(imei, MatchesRegularExpression.matchesRegularExpression("\\A[\\d\\.\\:\\-\\s]+\\z"));
        assertThat(LuhnCheckDigit.LUHN_CHECK_DIGIT.isValid(imei), is(true));
    }

    @Test
    public void ean8() {
        assertThat(faker.code().ean8(), MatchesRegularExpression.matchesRegularExpression("\\d{8}"));
    }

    @Test
    public void gtin8() {
        assertThat(faker.code().gtin8(), MatchesRegularExpression.matchesRegularExpression("\\d{8}"));
    }

    @Test
    public void ean13() {
        String ean13 = faker.code().ean13();
        assertThat(ean13, MatchesRegularExpression.matchesRegularExpression("\\d{13}"));
    ...
    public void gtin13() {
        ...
        assertThat(gtin13, MatchesRegularExpression.matchesRegularExpression("\\d{13}"));
